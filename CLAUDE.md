...
| `#projects` | 04 — **10 hand-written** `<article class="card">`: one full-width `.feat` (Wind-Farm, 32px mono metric, `.ftag`) + 9 in the 2-col `.proj-grid` (last: Deutsch-Tutor, whose card-link goes to the live app https://tutor.aravindpradee.me, not GitHub). Title + one-line pitch + metric + chips + Details ▾. Not API-driven. |
